...
<script type="text/javascript">
	function fn_order_each_goods(productNum, productName, productPrice,
			productSale, fileName, productDelivery) {
		...
		var i_option = document.createElement("input");
		...
		i_productSale.value = productSale;
		i_orderAmount.value = productSale;
		i_fileName.value = fileName;
		i_productDelivery.value = productDelivery;
		if (option2 == "n") {
			i_option.value = option;
		} else {
			i_option.value = option + "/" + option2;
		}
	...
	<div class="layout">
		<div class="productdetail">
			<div class="productimage">
				<img
					src="${contextPath}/thumbnails.do?productNum=${goods.productNum}&fileName=${goods.fileName}"
					width="360" height="360" /><br> <img
					src="${contextPath}/thumbnails.do?productNum=${goods.productNum}&fileName=${goods.fileName}"
			...
			<div class="product">
				...
				<table class="product_detail">
					<tbody>
						<tr>
							<th>원가</th>
							<td>${goods.originPrice}원</td>
						</tr>
						<tr>
							<th>판매가</th>
							<td>${goods.productPrice}원</td>
						</tr>
						<tr>
							<c:if test="${optionlist[0]!=null}">
								<th>옵션</th>
								<td><select id="option">
							...
							<c:if test="${optionlist[0]==null}">
								<input type="hidden" id="option" value="n">
							</c:if>
						</tr>
						<tr>
							<c:if test="${optionlist2[0]!=null}">
								<th>옵션</th>
								<td><select id="option2">
										<option value="notSelect">-</option>
										<c:forEach var="item" items="${optionlist2 }">
											<option value="${item }">${item }</option>
										</c:forEach>
								</select></td>
							</c:if>
							<c:if test="${optionlist2[0]==null}">
								<input type="hidden" id="option2" value="n">
							</c:if>
						</tr>
						<tr>
							<th>수량</th>
							<td><select id="number_qty">
				...
				<div class="productbutton">
					<c:choose>
						<c:when test="${goods.productsStock == 0}">
							<div class="buy">
								<a href=""><input type="button" value="바로구매"
									style="font-size: 20px" disabled="disabled" /></a>
							</div>
							<div class="cart">
								<a href=""><input type="button" value="장바구니"
									style="font-size: 20px" disabled="disabled" /></a>
							</div>
						...
						<c:otherwise>
							<div class="buy">
								<a
									href="javascript:fn_order_each_goods('${goods.productNum }','${goods.productName }','${goods.originPrice}','${goods.productPrice}','${goods.fileName}',${goods.productDelivery });">바로구매</a>
							</div>
							<div class="cart">
								<a href="javascript:add_cart('${goods.productNum }')">장바구니
									담기</a>
							</div>
						</c:otherwise>
					</c:choose>
			...
			<div id="tab-1" class="tab-content current">
				<div class="productintro">
					<c:forEach var="image" items="${imageList }">
						<img
							src="${contextPath}/download.do?productNum=${goods.productNum}&fileName=${image.fileName}">
					</c:forEach>
				</div>
			...
			<div id="tab-3" class="tab-content">
				<div id="reviewarticle">
					...
					<div class="image">
						<img src="${contextPath}/resources/image/item1.JPG" width="130"
							height="130" /> <img
							src="${contextPath}/resources/image/item1.JPG" width="130"
							height="130" /> <img
							src="${contextPath}/resources/image/item1.JPG" width="130"
							height="130" /> <img
							src="${contextPath}/resources/image/item1.JPG" width="130"
				...
				<div id="story" style="display: none">
					...
					<div class="writeinfo3">
						<form class="mb-3" id="myform" action="#" method="post"
							...
							<table>
								...
								<tr>
									...
									<td id="detail_title2"><input type="radio" name="open"
										checked />공개 <input type="radio" name="open" />비공개</td>
								</tr>
								...
								<tr>
									...
									<td id="detail_title2">
										<fieldset>
											<input class="reviewStar" type="radio" name="reviewStar"
												value="5" id="rate1"> <label for="rate1">★</label> <input
												class="reviewStar" type="radio" name="reviewStar" value="4"
												id="rate2"> <label for="rate2">★</label> <input
												class="reviewStar" type="radio" name="reviewStar" value="3"
												id="rate3"> <label for="rate3">★</label> <input
												class="reviewStar" type="radio" name="reviewStar" value="2"
												id="rate4"> <label for="rate4">★</label> <input
												class="reviewStar" type="radio" name="reviewStar" value="1"
								...
								<tr>
									...
									<td id="detail_title2"><input type="button" value="파일 추가"
										onClick="fn_addFile()" /> <span id="input_file"></span></td>
								</tr>

								<tr>
									<td id="detail_title">내용</td>
									<td id="detail_title2"><textarea class="detail_text"
											name="detail_text"></textarea></td>
								</tr>
			...
			<div id="tab-4" class="tab-content">
				<div id="qnaarticle">
					...
					<div class="reviewtitle">
						...
						<div class="reviewtitle2">
							<a style="CURSOR: hand"
								onclick="if(reviewtitle.style.display=='none') {reviewtitle.style.display='';} else {reviewtitle.style.display='none';}">캐리어
								크기 문의 드립니다.</a>
						</div>
					...
					<div class="reviewtitle" id="reviewtitle" style="display: none">
						...
						<div class="qnaanswer">
							...
							<br>
							<p>답변:</p>
							<p>안녕하세요 고객님. 내일갈래입니다.안녕하세요 고객님. 내일갈래입니다.안녕하세요 고객님.
								내일갈래입니다.안녕하세요 고객님. 내일갈래입니다.안녕하세요 고객님. 내일갈래입니다.안녕하세요 고객님.
								내일갈래입니다.</p>
						</div>
				...
				<div id="write_box" class="write">
					<a style="CURSOR: hand" onclick="qna_fnc()">문의하기</a>
				</div>
				<div id="writeqna" style="display: none">

					<div class="writeinfo3">
						<form id="qnaform" action="#" method="post" name="qnaform"
							...
							<table>
								...
								<tr>
									<td id="detail_title">파일 첨부</td>
									<td id="detail_title2"><input type="button" value="파일 추가"
										onClick="fn_addFile2()" /> <span id="input_file2"></span></td>
								</tr>
								...
								<tr align="center" valign="middle">
									<td colspan="5"><input type="submit" value="문의하기">
		...
		function fn_addFile() {
			if (cnt == 0) {
				$("#input_file")
						.append(
								"<br>"
										+ "<input type='file' name='main_image' id='in_main_image' />");
			} else {
				$("#input_file")
						.append(
								"<br>"
