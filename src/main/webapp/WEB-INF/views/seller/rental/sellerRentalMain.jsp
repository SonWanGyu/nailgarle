...
.seller_container .sidebar .insellerRental {
	font-weight: 800;
}
...
.seller_rental_container {
	text-align: center;
}

.seller_rental_container h4 {
	font-size: 12px;
	text-align: left;
	margin-top: 12px;
	padding-left: 300px;
...
.seller_rental_container table {
	width: 950px;
}

/* 판매자 페이지 렌탈 관리 tr, th, td 태그 */
.seller_container .sellerdata_rental th {
	background: #F4A460;
	color: #fff;
}

.seller_container .sellerdata_rental td, th {
	padding: 5px 20px;
	font-size: 12px;
}

.seller_container .sellerdata_rental tr:nth-of-type(odd) {
	background: #eee;
}
...
<div class="layout">
			...
			<div class="sidebar">
				<nav>
					<ul>
						...
						<li><a class="insellerRental" href="${contextPath}/seller/rental/sellerRentalMain.do"><img class="seller_img" src="${contextPath}/resources/image/seller_rental.svg" />렌탈 관리</a></li>
		...
		<div class="seller_rental_container">
				<h4>렌탈조회</h4>
			...
			<!-- search -->
				<form name="seller_order_container_search">
								...
								<td>
									...
									<div class="keywordform" style="margin-top: 10px;">
										<select id="searchType" class="searchType" name="searchType">
											...
											<option value="rentalType">반납상태</option>
					...
					<table>
						...
						<tbody align="center">
							...
							<tr>
								...
								<td><a class="seller_product" href="${contextPath}/goods/productdetail.do?productNum=${item.productNum }">${item.productName }</a></td>
					...
					<table class="all_seller_rental">
						<tbody align="center">
							<tr>
								<td colspan="7" class="page_nation" style="padding-top: 20px;">
									<c:forEach var="page" begin="1"	end="10" step="1">
										<c:if test="${section >1 && page==1 }">
											<a class="arrow prev" href="${contextPath}/seller/rental/sellerRentalMain.do?section=${section-1}&pageNum=${(section-1)*10-9}"></a>
										</c:if>
											<a class="active" href="${contextPath}/seller/rental/sellerRentalMain.do?section=${section}&pageNum=${page}">${(section-1)*10 +page }</a>
										<c:if test="${page ==10 }">
											<a class="arrow next" href="${contextPath}/seller/rental/sellerRentalMain.do?section=${section+1}&pageNum=${section*10+1}"></a>
										</c:if>
									</c:forEach>
								</td>
							</tr>
