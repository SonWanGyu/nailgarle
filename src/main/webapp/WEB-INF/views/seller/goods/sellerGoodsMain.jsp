...
<script>
	...
	function fn_seller_goods_search() {
		var seller_goods_search = document.seller_goods_search;

		searchType = seller_goods_search.searchType.value;
		keyword = seller_goods_search.keyword.value;
		beginDate = seller_goods_search.searchStartDate.value;
		endDate = seller_goods_search.searchEndDate.value;
		...
		i_command.value = "seller_goods_search";
		...
		formObj.method = "get";
		formObj.action = "${contextPath}/seller/goods/sellerGoodsMain.do";
		formObj.submit();
	}
</script>
...
.seller_goods_container {
	text-align: center;
}

.seller_goods_container h4 {
	font-size: 12px;
	text-align: left;
	margin-top: 12px;
	padding-left: 300px;
...
.seller_goods_container table {
	width: 950px;
}

/* 판매자 페이지 물품 관리 tr, th, td 태그 */
.seller_container .sellerdata_goods th {
	background: #F4A460;
	color: #fff;
}

.seller_container .sellerdata_goods td, th {
	padding: 5px 20px;
	font-size: 12px;
}

.seller_container .sellerdata_goods tr:nth-of-type(odd) {
	background: #eee;
}
...
<body>
	<div class="layout">
		...
		<div class="seller_container">
			...
			<div class="seller_goods_container">
				<h4>물품조회</h4>
				
				<!-- search -->
				<form name="seller_goods_search">
								...
								<td>
									...
									<div class="keywordform" style="margin-top: 10px;">
										<select id="searchType" class="searchType" name="searchType">
											...
											<option value="allCategory">카테고리</option>
										</select> 
											<input type="text" class="keyword" id="keyword" name="keyword" /> 
											<input type="button" class="search_btn" onClick="fn_seller_goods_search()">
									</div>
								</td>
							</tr>
						<tbody>
			...
			<div class="sellerdata_goods">
					...
					<tbody align="center">
						...
						<c:choose>
							<c:when test="${empty sellerGoodsList }">
								<tr>
									<td colspan="8"><strong>등록하신 상품이 없습니다.</strong></td>
								</tr>
							</c:when>
							<c:otherwise>
								<c:forEach var="item" items="${sellerGoodsList}" varStatus="i">
									<tr>
										<td>${item.productNum }</td>
										<td>${item.mainCategoryName }</td>
										<td>${item.subCategoryName }</td>
										<td><a class="seller_product" href="${contextPath}/seller/goods/modifyGoodsForm.do?productNum=${item.productNum }">${item.productName }</a></td>
				...
				<table class="all_seller_goods">
					<tbody align="center">
						<tr>
							<td align="left" style="padding-top: 20px;">
								<input type="button" value="상품 등록" onclick="location.href='${contextPath}/seller/goods/sellerAddGoods.do'">
							</td>
						</tr>
						<tr>
							<td colspan="8" class="page_nation" style="padding-top: 20px;">
								<c:forEach var="page" begin="1" end="10" step="1">
									<c:if test="${section >1 && page==1 }">
										<a class="arrow prev" href="${contextPath}/seller/goods/sellerGoodsMain.do?.docommand=${command}&beginDate=${beginDate}&endDate=${endDate}&searchType=${searchType}&keyword=${keyword}&section=${section-1}&pageNum=${(section-1)*10-9}"></a>
									</c:if>
										<a class="active" href="${contextPath}/seller/goods/sellerGoodsMain.do?command=${command}&beginDate=${beginDate}&endDate=${endDate}&searchType=${searchType}&keyword=${keyword}&section=${section}&pageNum=${page}">${(section-1)*10 +page }</a>
									<c:if test="${page ==10 }">
										<a class="arrow next" href="${contextPath}/seller/goods/sellerGoodsMain.do?command=${command}&beginDate=${beginDate}&endDate=${endDate}&searchType=${searchType}&keyword=${keyword}&section=${section+1}&pageNum=${section*10+1}"></a>
									</c:if>
								</c:forEach>
							</td>
						</tr>
