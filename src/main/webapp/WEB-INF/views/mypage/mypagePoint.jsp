...
<script>
	...
	function point_detail_search() {
		var my_point_detail_list = document.my_point_detail_list;

		searchType = my_point_detail_list.searchType.value;
		keyword = my_point_detail_list.keyword.value;
		beginDate = my_point_detail_list.searchStartDate.value;
		endDate = my_point_detail_list.searchEndDate.value;
		...
		i_command.value = "point_detail_search";
		...
		formObj.method = "get";
		formObj.action = "${contextPath}/mypage/mypagePoint.do";
		formObj.submit();
	}
...
.mypage_container .sidebar .inmypagePointList {
	font-weight: 800;
}
...
.point_container table {
	width: 950px;
}

/* 마이페이지 주문/배송의 tr, th, td 태그 */
.point_container .mydata_point th {
	background: #FFC0CB;
	color: #fff;
}

.point_container .mydata_point td, th {
	padding: 5px 20px;
	font-size: 12px;
}

.point_container .mydata_point tr:nth-of-type(odd) {
	background: #eee;
}
...
/* page */
.point_container .mydata_point .page_nation {
	border: none;
	padding-top: 30px;
...
<div class="layout">
			...
			<div class="sidebar">
				<nav>
					<ul>
						...
						<li><a class="inmypagePointList" href="${contextPath}/mypage/mypagePoint.do"><img class="mypage_img" src="${contextPath}/resources/image/mypage_point.svg"/>포인트</a></li>
		...
		<div class="point_container">
				...
				<!-- search -->
				<form name="my_point_detail_list">
								...
								<td>
									...
									<div class="keywordform" style="margin-top: 10px;">
										<select id="searchType" class="searchType" name="searchType">
											...
											<option value="pointContents">내용</option>
											...
											<input type="button" class="search_btn" onClick="point_detail_search()">
									</div>
								</td>
							</tr>
						<tbody>
					...
					<tbody align="center">
						...
						<c:choose>
							<c:when test="${empty myPointList }">
								<tr>
									<td colspan="5"><strong>포인트 사용 내역이 없습니다.</strong></td>
								</tr>
							</c:when>
							<c:otherwise>
								<c:forEach var="item" items="${myPointList }" varStatus="i">
									<tr>
										<td>${item.recNum }</td>
										<td>${item.pointContents }</td>
										<td>${item.pointSave }</td>
										<td>${item.pointUse }</td>
										<td>${item.pointUseDate }</td>
								</c:forEach>
							</c:otherwise>
						</c:choose>
						...
						<tr>
							<td colspan="5" class="page_nation"><c:forEach var="page"
									...
									<c:if test="${section >1 && page==1 }">
										<a class="arrow prev" href="${contextPath}/mypage/mypagePoint.do?command=${command}&beginDate=${beginDate}&endDate=${endDate}&searchType=${searchType}&keyword=${keyword}&section=${section-1}&pageNum=${(section-1)*10-9}"></a>
									</c:if>
										<a class="active" href="${contextPath}/mypage/mypagePoint.do?command=${command}&beginDate=${beginDate}&endDate=${endDate}&searchType=${searchType}&keyword=${keyword}&section=${section}&pageNum=${page}">${(section-1)*10 +page }</a>
									<c:if test="${page ==10 }">
										<a class="arrow next" href="${contextPath}/mypage/mypagePoint.do?command=${command}&beginDate=${beginDate}&endDate=${endDate}&searchType=${searchType}&keyword=${keyword}&section=${section+1}&pageNum=${section*10+1}"></a>
									</c:if>
								</c:forEach>
							</td>
						</tr>
